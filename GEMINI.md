...
- **프레임워크:** 전자정부 프레임워크 (eGovFrame 5.x) 기반
- **들여쓰기:** 스페이스 4칸 (HTML/CSS/JS) tab으로 열맞추기 (Java)
...
작업이 완료될 때마다 프로젝트 루트(`E:\dev\front\did_emart\frint`)에  
`[GEMIN]YYYY-MM-DD.md` 파일에 항목을 추가한다.

파일이 없으면 `#[GEMIN]YYYY-MM-DD 작업 로그` 헤더로 새로 생성한다.
